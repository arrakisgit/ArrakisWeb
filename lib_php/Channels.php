...
class FranceTV extends ScrappingCURL implements IChannel
{
	...
	public function Categories()
	{
		...
		/*foreach($this->JSON_RESULT_CATEGORIES_FRANCETV['categories'][0] as $categories)
		{
			foreach($categories['genres'][0] as $genres)
			{
		...
		return var_dump($this->JSON_RESULT_CATEGORIES_FRANCETV['categories'][1]['genres']);//$ARRAY_CATEGORIES;
	}
